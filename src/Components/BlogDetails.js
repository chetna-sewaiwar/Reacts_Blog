...
const typeObj = {  // contains all utility array based on type,for eg:if type is travel,then utility is places
  Travel: places,
  Food : food,
  Bollywood :  bollywood,
  Hollywood : hollywood,
...
const BlogDetails = () => {
  ...
  const [selectedBlog, setSelectedBlog] = useState({ //setting the selected blog by finding the id and when id matches,it stores in that state
    id,
    type,
    url: "",
    about: "",
  ...
  useEffect(() => {
    setSelectedBlog(() => {
      let blog = typeObj[type].find(    //based on typeObj created above,we will get the array which we need to loop nd then find the id which matches our url id  
